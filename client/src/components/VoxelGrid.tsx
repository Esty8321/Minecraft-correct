...
<<<<<<< HEAD
  ...
  const livePlayers = useMemo(() => {
   ...
   return arr.map((p: any) => ({
    id: p.id ?? p.player_id ?? String(p.user_id ?? ""),
    username: p.username ?? p.name ?? p.id ?? "player",
    email: p.email ?? "",
    row: p.row ?? p.pos?.row ?? 0,
     col: p.col ?? p.pos?.col ?? 0,     chunk_id: p.chunk_id ?? chunkId,
   }));
 }, [players, gameState?.chunk_id]);
...
<<<<<<< HEAD
              ...
              currentChunkId={gameState?.chunk_id ??
                sessionStorage.getItem("current_chunk_id") ??
                null}
              playersLive={livePlayers}          
                />
=======
              playerId={myId}
              currentChunkId={
