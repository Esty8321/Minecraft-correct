...
class NextActionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            board2ch,                           # (2,H,W)
            int(s["prev_token"]),               # prev_token
            int(uidx),                          # user_idx
            int(s["target_idx"]),               # target class idx
            float(s["prev_delta_norm"]),        # prev_delta_norm (input)
            float(s["sleep_delta_norm"]),       # next_delta_norm (target for SLEEP)
        )
...
def train(
    ...
    def _batch_to_device(batch):
        ...
        x  = board2ch.float().to(device)              # (B,2,H,W) – נרמול יעשה ב-CNN
