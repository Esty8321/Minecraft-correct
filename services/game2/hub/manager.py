...
class Hub:
    ...
    def __init__(self, world: WorldService, movement: MovementService, 
        ...
        # ✨ NEW: נשתמש במיפוי לשמירת סנפשוט של בוט כאשר המשתמש מתחבר (כדי להחזיר בלוגאאוט)
        # user_id -> {"h": Optional[Tensor], "last_token": int}
        self._bot_snaps: dict[str, dict] = {}
    ...
    async def connect(self, ws: WebSocket) -> None:
        ...
        # ✨ CHANGED: אם בוט רץ — עוצרים ושומרים סנפשוט (state יגיע מהסשן או מהבוט)
        snap = None
        if self.bots.is_running(user_id):
            snap = self.bots.stop(user_id)  # BotSnapshot(state, h, last_token)
            if snap:
                self._bot_snaps[user_id] = {"h": snap.h, "last_token": snap.last_token}

        user_sockets = self.sessions.sockets_for_user(user_id)
        if user_sockets:
            # כבר קיים חיבור כלשהו — נשתמש ב-state של הסשן הקיים
            any_ws = next(iter(user_sockets))
            existing_session = self.sessions.get(any_ws)
            state = existing_session.state
        else:
            # אין חיבור קיים: אם היה בוט — ניקח את ה-state מהסנפשוט, אחרת spawn חדש
            if snap is not None:
                state = snap.state
            else:
                chunk_id, spawn = await self.world.get_spawn_position(user_id)
    ...
    async def disconnect(self, ws: WebSocket) -> None:
        try:
            ...
            if not remaining:
                # ✨ CHANGED: לא עושים despawn אם מפעילים בוט - הבוט צריך להמשיך מאותו state
                # נשחזר h/last_token אם שמרנו אותם בהתחברות
                snap_data = self._bot_snaps.pop(user_id, None)
                h = snap_data["h"] if snap_data else None
                last_token = snap_data["last_token"] if snap_data else 0

                # מפעילים את הבוט על ה-state הנוכחי של השחקן (נשאר בעולם)
                self.bots.start(user_id, sess.state, h=h, last_token=last_token)
        except Exception as e:
            import traceback
            traceback.print_exc()
    ...
    async def move(self, ws: WebSocket, dr: int, dc: int) -> None:
        ...
        self.world.player_actions_history.record_player_action(
            state.user_id, state.chunk_id, dr, dc, board, players=players_now
        )
        if moved.old_chunk_id and moved.old_chunk_id != state.chunk_id:
            self.sessions.update_watchers_after_chunk_change(state.user_id, moved.old_chunk_id, state.chunk_id)
            await self.scrolls.broadcast_chunk(state.chunk_id)
    ...
    async def whereami(self, ws: WebSocket) -> None:
        ...
        # ✨ FIX: שליחה על מופע ה-WS ולא על המחלקה
