...
hub = Hub(
    world_service, movement_service, scroll_service, bot_service,
    session_store, color_service, player_db, chunk_players
)
...
# ✨ טוען את משקלי הבוט בעליית השרת כדי למנוע דיליי בטיקט הראשון
@app.on_event("startup")
async def _startup_load_bot():
    try:
        bot_service.load_model("bot_gru.pt")
        logger.info("[startup] bot model loaded")
    except Exception as e:
        logger.warning(f"[startup] bot model not loaded: {e}")
...
async def ws_endpoint(ws: WebSocket) -> None:
    ...
    try:
        while True:
            ...
            if player_id is None:
                # ✨ לוג תקין (log(level,msg)) במקום logger.log("string")
                logger.warning("missing player_id")
